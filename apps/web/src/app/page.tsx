...
export default async function Home() {
	...
	return (
		<div className="dark relative min-h-svh bg-[#060606] text-white selection:bg-white/10">
			{/* ── Nav ── */}
			...
			<section className="relative px-6 pb-32 pt-16 lg:px-16 lg:pt-24">
				...
				<div className="relative mx-auto grid max-w-6xl items-center gap-16 lg:grid-cols-2 lg:gap-20">
					...
					<div className="animate-fade-in-up max-w-lg">
						...
						<p
							className="animate-fade-in-up mt-6 max-w-sm text-[15px] leading-relaxed text-white/40"
						...
						>
							<Link
								href="/signin"
								className="inline-flex h-12 items-center rounded-full bg-white px-8 text-[13px] font-semibold text-[#060606] transition-opacity hover:opacity-90"
							>
								Join for free
							</Link>
							<Link
								href="/explore"
								className="inline-flex h-12 items-center rounded-full border border-white/10 px-8 text-[13px] font-medium text-white/60 transition-colors hover:border-white/20 hover:text-white/80"
							>
								Explore
							</Link>
						</div>
						<p
							className="animate-fade-in-up mt-5 text-[13px] text-white/25"
					...
					>
						{/* Back poster */}
						<div
							className="absolute right-20 top-4 w-[155px] overflow-hidden rounded-xl shadow-2xl shadow-black/50"
						...
						{/* Middle poster */}
						<div
							className="absolute right-40 top-16 w-[165px] overflow-hidden rounded-xl shadow-2xl shadow-black/50"
						...
						{/* Front poster */}
						<div
							className="absolute right-6 top-28 w-[175px] overflow-hidden rounded-xl shadow-2xl shadow-black/50"
						...
						{/* Match pill */}
						<div
							className="absolute bottom-20 left-0 flex items-center gap-2.5 rounded-full border border-white/10 bg-[#111] px-4 py-2.5 shadow-xl"
							style={{ transform: "rotate(-2deg)" }}
						>
							<div className="flex -space-x-2">
								<div className="flex size-6 items-center justify-center rounded-full bg-amber-500/20 text-[9px] font-bold text-amber-300 ring-2 ring-[#111]">
									S
								</div>
								<div className="flex size-6 items-center justify-center rounded-full bg-sky-500/20 text-[9px] font-bold text-sky-300 ring-2 ring-[#111]">
									J
								</div>
							</div>
							<span className="text-xs text-white/50">3 matches</span>
						</div>

						{/* Watchlist badge */}
						<div
							className="absolute bottom-36 right-0 flex items-center gap-1.5 rounded-full border border-white/10 bg-[#111] px-3.5 py-2 shadow-xl"
							style={{ transform: "rotate(3deg)" }}
						>
							<Bookmark className="size-3 text-white/50" />
							<span className="text-xs text-white/50">Watchlist</span>
						</div>
					</div>
				</div>
			</section>
				...
				<div className="mx-auto max-w-6xl space-y-40">
					...
					<div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
						...
						<RevealOnScroll direction="right" delay={150}>
							<div>
								<p className="mb-3 flex items-center gap-2 font-display text-xs font-medium uppercase tracking-[0.2em] text-white/20 sm:gap-3">
			...
			<section className="relative mt-20 px-6 pb-20 pt-36 lg:px-16">
				...
				<div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
					<div className="h-[400px] w-[600px] rounded-full bg-white/[0.02] blur-[120px]" />
				</div>
					...
					<div className="relative mx-auto flex max-w-lg flex-col items-center text-center">
						...
						<div className="mb-8 flex items-center gap-1.5">
							{[...Array(5)].map((_, i) => (
								<div
									key={i}
									className="size-2 rounded-[3px] border border-white/[0.08]"
						...
						<p className="mt-5 max-w-sm text-[15px] leading-relaxed text-white/35">
							Miru is free to use. Create an account, build your watchlist, and
							start finding movies to watch with friends.
						</p>
						<Link
							href="/signin"
							className="group mt-10 inline-flex h-12 items-center gap-2 rounded-full bg-white px-10 text-[13px] font-semibold text-[#060606] transition-all hover:shadow-[0_0_30px_rgba(255,255,255,0.08)]"
						>
			...
			{/* ── Footer ── */}
			<footer className="mt-20 border-t border-white/[0.06] px-6 py-10 lg:px-16">
				<div className="mx-auto max-w-6xl">
					...
					<p className="mt-6 text-center text-[11px] text-white/25 sm:text-left">
		...
		<div className="mx-auto max-w-6xl px-6 lg:px-16">
			<div className="relative h-px">
				...
				<div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
					<div className="size-1.5 rotate-45 bg-white/10" />
				</div>
			</div>
		</div>
	);
...
function UserPill({ name, color }: { name: string; color: string }) {
	return (
		<div
			className={`flex size-7 items-center justify-center rounded-full text-[10px] font-bold ring-2 ring-[#0c0c0c] ${color}`}
		>
			{name.charAt(0)}
		</div>
	);
	...
	return (
		<div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
			<div className="flex items-center justify-between px-1">
				<p className="text-sm font-medium text-white/70">My Watchlist</p>
				<p className="text-xs text-white/25">12 movies</p>
			</div>
			<div className="mt-4 grid grid-cols-3 gap-2">
				{movies.map((m) => (
					<div key={m.title} className="space-y-1.5">
						<PosterThumb path={m.path} alt={m.title} className="aspect-[2/3]" />
						<p className="truncate px-0.5 text-[10px] text-white/30">
							{m.title}
						</p>
					</div>
				))}
...
function MockForYou() {
	...
	return (
		<div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
			<div className="px-1">
				<p className="text-sm font-medium text-white/70">For You</p>
				<p className="text-[11px] text-white/25">Based on 5 friends</p>
			</div>
			<div className="mt-4 space-y-3">
				{recommendations.map((m) => (
					<div
						key={m.title}
						className="flex items-center gap-3 rounded-xl bg-white/[0.02] p-2.5"
					>
						<PosterThumb
							path={m.path}
							alt={m.title}
						...
						<div className="min-w-0 flex-1">
							...
							<p className="text-[11px] text-white/25">{m.year}</p>
						</div>
						<div className="flex -space-x-1.5">
							{m.friends.map((f, i) => (
								<div
									key={f}
									className={`flex size-5 items-center justify-center rounded-full text-[8px] font-bold ring-1 ring-[#0c0c0c] ${m.friendColors[i]}`}
								>
									{f}
								</div>
							))}
